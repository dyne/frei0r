...
void rings(float *sl, int w, int h, float a, float ar, int linp, float sf, float ef)
{
  ...
  if (linp == 0)
  {
    ...
    for (x = 0; x < w * h; x++)
    {
      sl[x] = g;                     //match background to outer rim
    }
    for (x = -rmax; x < rmax; x++)
    {
      for (y = -rmax; y < rmax; y++)
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index)
{
  ...
  case 2:                                                                                //ver
                                                                                         ...
                                                                                         // ver
                                                                                         // sweep
    sweep_v(inst->sl, inst->w, inst->h, 1, inst->amp, inst->linp, inst->par, 0.05, 0.7); //ver
                                                                                         // f
                                                                                         // 
                                                                                         // ver
                                                                                         // sw
