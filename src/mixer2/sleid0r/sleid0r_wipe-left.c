...
void f0r_get_plugin_info(f0r_plugin_info_t *info)
    ...
    info->num_params = 1;
    info->explanation = "Wipe from right to left";
    }
...
void f0r_update2(
    ...
    off = (int)((inst->w + inst->bw) * inst->pos + 0.5) - inst->bw;
    bw = inst->bw;
    dbw = 0;
    ...
    for(y = 0; y < inst->h; ++y)
        ...
        c1 = (uint8_t *)(inframe1 + inst->w * y + inst->w - off - bw);
        c2 = (uint8_t *)(inframe2 + inst->w * y + inst->w - off - bw);
        co = (uint8_t *)(outframe + inst->w * y + inst->w - off - bw);
        for(i = 0; i < bw * 4; ++i)
            {
            unsigned k = inst->bwk[i / 4 + dbw];
        ...
        memcpy(outframe + inst->w * y + inst->w - off, inframe2 + inst->w * y + inst->w - off,
            off * sizeof(*outframe));
        }
    }
