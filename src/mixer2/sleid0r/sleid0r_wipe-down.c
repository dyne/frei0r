...
void f0r_get_plugin_info(f0r_plugin_info_t *info)
{
  ...
  info->num_params     = 1;
  info->explanation    = "Wipe from top to bottom";
}
  ...
  )
{
  ...
  memcpy(outframe, inframe2, inst->w * off * sizeof(*outframe));

  memcpy(outframe + inst->w * (off + bw), inframe1 + inst->w * (off + bw),
         inst->w * (inst->h - off - bw) * sizeof(*outframe));

  c1 = (uint8_t *)(inframe1 + inst->w * off);
  c2 = (uint8_t *)(inframe2 + inst->w * off);
  co = (uint8_t *)(outframe + inst->w * off);
  for (y = 0; y < bw; ++y)
  {
