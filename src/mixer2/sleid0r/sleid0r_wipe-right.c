...
void f0r_get_plugin_info(f0r_plugin_info_t *info)
    ...
    info->num_params = 1;
    info->explanation = "Wipe from left to right";
    }
...
void f0r_update2(
    ...
    for(y = 0; y < inst->h; ++y)
        {
        memcpy(outframe + inst->w * y, inframe2 + inst->w * y,
            off * sizeof(*outframe));

        c1 = (uint8_t *)(inframe1 + inst->w * y + off);
        c2 = (uint8_t *)(inframe2 + inst->w * y + off);
        co = (uint8_t *)(outframe + inst->w * y + off);
        for(i = 0; i < bw * 4; ++i)
            {
