...
void f0r_get_plugin_info(f0r_plugin_info_t *info)
{
  ...
  info->num_params     = 1;
  info->explanation    = "Slide from top to bottom";
}
  ...
  )
{
  ...
  memcpy(outframe, inframe2 + (inst->h - off) * inst->w,
         off * inst->w * sizeof(*outframe));

  for (xy = off * inst->w; xy < (off + bw) * inst->w; ++xy)
  {
    px = (inframe1[xy] >> 2) & 0x3F3F3F3F;
    ((uint8_t *)(&px))[3] = ((uint8_t *)(inframe1 + xy))[3];
    outframe[xy]          = px;
  }

  memcpy(outframe + (off + bw) * inst->w, inframe1 + (off + bw) * inst->w,
         (inst->h - off - bw) * inst->w * sizeof(*outframe));
}
