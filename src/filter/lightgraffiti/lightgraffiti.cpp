 ...
 *
 *
 * This effect is intended to simulate what happens when you use a shutter speed
 * of
 * e.g. 10 seconds for your camera and paint with light, like lamps, in the air
 * -- just with video. It tries to remember bright spots and keeps them in a
 * mask.
 * Areas that are not very bright (i.e. background) will not sum up.
 *
 * Originally I saw this effect in some Ford Kuga commercials on YouTube when a
 * friend
 * shew me those. One of them was [1]. No information about how this effect
 * works is given
 * -- only that a guy from the Dutch PIPS:LAB[2] was involved. The technique
 * seems to be
 * slightly different though; whileas this frei0r effect works in post, the
 * original Lumasol
 * effect is said to work directly in-camera.
 *
 * Since the technique is fascinating I started writing this Open-Source
 * effect.[3] The general
 * concept is:
 * 1. Extract the light by using thresholding (absolute brightness and
 * brightness change relative to
 *   the background image fetched from the first frame)
 * 2. Store the color in a light mask, and the estimated density in an alpha map
 * (increased every time
 *   that a light source hits a pixel to simulate overexposure)
 * 3. Paint the light mask over the video image
 * 4. Dim the alpha map, and update the background image (moving average), if
 * desired.
 * 5. Repeat for the next frame.
 *
 * The second approach (LG_ADV) is based on the observation that colour mixing
 * does not work well
 * with the above one that stores colour values and changes the brightness via
 * an alpha map. Therefore
 * the new approach directly sums up colour values detected in the light source
 * and does not use an
 * alpha map.
 * Transitions are not very smooth out-of-the-box. This is solved by multiplying
 *the light source's
 *  RGB value by (r+g+b)/3 (after normalizing them to [0,1]); Darker lights will
 *then be even darker
 *  and the transition to the background looks smoother.
 * Lights may look a little faint regarding color. Therefore the saturation can
 *be increased by a custom
 *  factor. Saturation depends on the brightness of the light map; the darker
 *the light is, the more
 *  the saturation is increased. This will, within a sensible range, make the
 *lights look more vital.
 * Dimming works by scaling each color values individually.
 *
 * If you write your own Light Graffiti effect (e.g. for After Effects) I'd very
 *much appreciate
 * to hear about it!
 *
 * -- Simon (Granjow)
...
public:
                      ...
                      const uint32_t *in)
  {
    ...
    if (!m_meanInitialized || m_pReset)
    {
      ...
      {
        // Do not use the first frame from the movie as background image but
        // plain black
        // to calculate the added light. Useful e.g. when dealing with still
        // images.
    ...
    {
      // Calculate the mean image to estimate the background. If alpha is set >
      // 0, bright light sources
      // moving into the image and standing still will eventually be treated as
      // background.
      if (m_pLongAlpha > 0)
      {
        for (unsigned int pixel = 0; pixel < width * height; pixel++)
    ...
    if (m_pDim > 0)
    {
      ...
      case Dim_Sin:
        ...
        for (size_t i = 0; i < m_rgbLightMask.size(); i++)
        {
          ...
          if (m_rgbLightMask[i].r < 0)
          {
            m_rgbLightMask[i].r = 0;
          }
          ...
          if (m_rgbLightMask[i].g < 0)
          {
            m_rgbLightMask[i].g = 0;
          }
          ...
          if (m_rgbLightMask[i].b < 0)
          {
            m_rgbLightMask[i].b = 0;
          }
        }
#else
        // Attention: Since Graffiti_LongAvgAlphaCumC only makes use of the
        // first alpha channel
...
#endif
      // m_longMeanImage has been handled above already (set to the current
      // image).
    }
    ...
    case Graffiti_S_Stat:
      for (unsigned int pixel = 0; pixel < width * height; pixel++)
      {
        min = GETR(out[pixel]);
        max = GETR(out[pixel]);
        if (GETG(out[pixel]) < min)
        {
          min = GETG(out[pixel]);
        }
        if (GETG(out[pixel]) > max)
        {
          max = GETG(out[pixel]);
        }
        if (GETB(out[pixel]) < min)
        {
          min = GETB(out[pixel]);
        }
        if (GETB(out[pixel]) > max)
        {
          max = GETB(out[pixel]);
        }
        if (min == 0)
    ...
    case Graffiti_SDiff_Stat:
      for (unsigned int pixel = 0; pixel < width * height; pixel++)
      {
        ...
        if (m_longMeanImage[3 * pixel + 1] < min)
        {
          min = m_longMeanImage[3 * pixel + 1];
        }
        if (m_longMeanImage[3 * pixel + 1] > max)
        {
          max = m_longMeanImage[3 * pixel + 1];
        }
        if (m_longMeanImage[3 * pixel + 2] < min)
        {
          min = m_longMeanImage[3 * pixel + 2];
        }
        if (m_longMeanImage[3 * pixel + 2] > max)
        {
          max = m_longMeanImage[3 * pixel + 2];
        }
        if (min == 0)
        {
          out[pixel] = 0;
    ...
    case Graffiti_LongAvgAlphaCumC:
       ...
       * Remember Hue if Saturation > 0.1 (below: Close to white, so Hue might
       *be wrong → remember Saturation as well)
       ...
       * If pixel is bright in another frame: Sum up alpha values (longer
       *exposure)
      ...
      for (unsigned int pixel = 0; pixel < width * height; pixel++)
      {
        ...
        {
#ifdef LG_ADV
          // Just add values as float. Overflows are highly unlikely (3.4E38+
          // frames ...).
          fr = CLAMP(r) / 255.0;
          fg = CLAMP(g) / 255.0;
          fb = CLAMP(b) / 255.0;
...
// fb2: " << fb << "\n";
...
#else
          // Store the «additional» light delivered by the light source in the
          // light mask.
          color = RGBA(CLAMP(r), CLAMP(g), CLAMP(b), 0xFF);
          m_lightMask[pixel] = MAX(m_lightMask[pixel], color);

          // Add the brightness of the light source to the brightness map (alpha
          // map)
        ...
        {
          // Use part of the background mean. This allows one to have only
          // lights appearing in the video
          // if people or other objects walk into the video after the first
          // frame (darker, therefore not in the light mask).
          ...
          )
        {
          ...
          if (lowerOverexposure > 0)
          {
            ...
            // clf;hold on;plot([0 1],[0
            // 1],'k');plot(range,ones(length(range),1),'k');plot(range,sqrt(range));plot(range,log(1+range),'k');plot(range,log(1+range),'g');plot(range,(log(1+range)/3).^.5,'r');axis
            // equal
          ...
          // A very bright red light source will eventually overflow into other
          // channels.
          sr = 0;
          sg = 0;
          sb = 0;
          ...
          // Increase the saturation if the average brightness is below a
          // certain level
          // Do not use Rec709 Luma since we want to consider all colours to
          // equal parts.
          fy = (fr + fg + fb) / 3;
        ...
        {
          // Show the image's brightness and highlight the threshold set by the
          // user
        ...
        if (m_pStatsDiff)
        {
          // As above, but for the brightness difference relative to the
          // background.
          r = .8 * CLAMP(maxDiff);
          g = r;
          if (!m_pStatsBrightness)
