                ...
                float max_space_dist, float dist_weight) //, float color_weight)
{
  ...
  //return sqrtf(color_weight*color_dist*color_dist +
  // dist_weight*space_dist*space_dist);
                ...
                const uint32_t *inframe, uint32_t *outframe)
{
  ...
  for (k = 0; k < inst->num; k++)
  {
    ...
    if (cc->numpix > 0)
    {
      ...
      //printf("%d, %d %d %d\t", k, (unsigned int)cc->r, (unsigned int)cc->g,
      // (unsigned int)cc->b);
      //printf("%g, %g %g %g\n", cc->numpix, cc->aggr_r, cc->aggr_g,
      // cc->aggr_b);
    }
