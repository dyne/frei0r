                         ...
                         f0r_param_t param, int param_index)
{
  ...
  default:
    if (param_index > 5)
    {
      inst->points[param_index - 6] = *((f0r_param_double *)param);   //Assigning
                                                                      // value
                                                                      // to
                                                                      // curve
                                                                      // point
                         ...
                         f0r_param_t param, int param_index)
{
  ...
  default:
    if (param_index > 5)
    {
      *((f0r_param_double *)param) = inst->points[param_index - 6];     //Fetch
                                                                        // curve
                                                                        // point
                                                                        // value
    }
...
double * gaussSLESolve(size_t size, double *A)
{
  ...
  for (int row = 0; row < size; row++)
  {
    ...
    while (A[row * extSize + col] == 0 && lastRowToSwap > row)               //checking
                                                                             // if
                                                                             // current
                                                                             // and
                                                                             // lower
...
double * calcSplineCoeffs(double *points, size_t pointsSize)
{
  ...
  else if (size == 3)           //coefficients of quadrant function Ax^2 + Bx +
                                // C = y
  {
    double *m = (double *)calloc(mxSize * extMxSize, sizeof(double));
    for (int i = 0; i < size; i++)
  ...
  else if (size > 3)           //coefficients of cubic spline Ax^3 + Bx^2 + Cx +
                               // D = y
  {
...
/**
 * Calculates a x,y pair for \param t on the cubic Bézier curve defined by
 *\param points.
 * \param t "time" in the range 0-1
 * \param points points[0] = point1, point[1] = handle1, point[2] = handle2,
 *point[3] = point2
 */
position pointOnBezier(double t, position points[4])
{
  ...
  /*
   * Calculating a point on the bezier curve using the coefficients from
   *Bernstein basis polynomial of degree 3.
   * Using the De Casteljau algorithm would be slightly faster when calculating
   *a lot of values
   * but the difference is far from noticable here since we update the spline
...
/**
 * Updates the color map according to the bézier spline described in the "Bézier
 *spline" parameter.
 */
void updateBsplineMap(f0r_instance_t instance)
{
  ...
  for (int i = 0; i < count - 1; ++i)
  {
    ...
    if (c == 0)
    {
      ...
      // calculate anyways, in case we only have these two points (with more
      // points this x value will be calculated three times)
      c = 1;
    }
    step = 1 / (double)c;
    ...
    k = 0;
    // connection points will be written twice (but therefore no special case
    // for the last point is required)
...
/**
 * Updates the color map according to the cubic spline described in the "Curve
 *Point" parameter.
 */
void updateCsplineMap(f0r_instance_t instance)
{
