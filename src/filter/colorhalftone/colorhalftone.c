  ...
  for (y = 0; y < height; y++)
  {
    for (channel = 0; channel < 3; channel++)
    {
      ...
      for (x = 0; x < width; x++)
      {
        ...
        // TODO: Efficiency warning: Because the dots overlap, we need to check
        // neighbouring grid squares.
      ...
      if (channel != 2)
      {
        dst = dst - width;// we're starting row again for next channel, unless
                          // this was last channel
      }
    }
  }
}
