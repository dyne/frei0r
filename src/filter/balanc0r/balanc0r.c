/* balanc0r.c
 * Copyright (C) 2009 Dan Dennedy <dan@dennedy.org>
 * This file is a Frei0r plugin based on digikam's white balance plugin
 * whose code is under the following copyrights.
 * Code in setRGBmult and f0r_set_param_value derived from code
 *   copyright (C) 2005-2006 Gilles Caulier <caulier dot gilles at kdemail dot
 *net>.
 * Values in bbWB array from code
...
{
  colordistance_info->name           = "White Balance";
  colordistance_info->author         = "Dan Dennedy";
