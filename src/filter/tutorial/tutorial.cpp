 ...
 *
 * The CMakeLists.txt needs to be adjusted as well (both in this directory and
...
public:
...

  {
    ...
    // Create the lookup table (see update()). Use std::vector instead of an
    // array here, since:
    ...
    // std::cerr << lookupTable.size() << " elements in the lookup table." <<
    // std::endl;

    // Calculate the entries in the lookup table. Applied on the R value in this
    // example.
    // If the R value of an input pixel is r, then the output R value will be
    // mapped to lookupTable[r].
    ...
    for (size_t i = 0; i < lookupTable.size(); i++)
    {
      ...
      f = pow(f, 2);                                       // Parabola
      f = f * .8 + h;                                      // Modification to
                                                           // the parabola

      // Since we might get negative values above, directly putting f into an
      // unsigned data type
      // may lead to interesting effects. To avoid this, use a signed integer
      // and clamp it to valid ranges.
    ...
    // This is a second lookup table for addition of two uint8 numbers. The
    // result of {0,255}+{0,255}
    // ranges in {0,511}. The usual way to clamp the values to the {0,255} range
    // is to use if/else
    // (if (k > 255) { k = 255; }), however using a lookup table is slightly
    // faster:
                      ...
                      const uint32_t *in)
  {
    ...
    // SOP/Sat effect uses pow(), division, and multiplication for the Power
    // parameter.
    // Since this only depends on the R/G/B value and not on previous frames,
    // the target value
    // can be pre-computed; when applying the filter, only thing left to do is
    // reading the value
    // in the lookup table.

    // This parameter allows one to do simple benchmarking: Rendering a
    // video with uint8_t pointers and with uint32_t pointers.
    // (Don't forget to substract the rendering time without this effect applied
    // to avoid counting
    ...
    {
      // This method takes only 80% of the time if only processing the R
      // channel,
