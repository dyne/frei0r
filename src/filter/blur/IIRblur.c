...
float AitNev3(int t, float xt[], float yt[], float x)
{
  float p[10];
  int   i, j, m;
  float zero = 0.0f;   // MSVC doesn't allow division through a zero literal,
                       // but allows it through non-const variable set to zero
...
void f0r_update(f0r_instance_t instance, double time, const uint32_t *inframe, uint32_t *outframe)
{
  ...
  case 2:
    ...
    // The bottom 3 lines were not updated, and outframe may be initialized with
    // garbage.
