...
//top left
  x = x0 + (np2 - sx / 2) * vp - 1;    if (sx > np)
  {
    x = x0;
  }
  y = y0 + (np2 - sy / 2) * vp - 1;    if (sy > np)
  {
    y = y0;
  }
  if (sx <= np)
  {
    draw_rectangle(s, w, h, x, y, 1, vp, white);
  }
  if (sy <= np)
  {
    draw_rectangle(s, w, h, x, y, vp, 1, white);
  }
//top right
