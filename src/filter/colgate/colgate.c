...
// Convert a linear RGB value in s6.25 fixed-point to sRGB (between 0 to 255,
// inclusive).
 ...
 * compute scaling factors for L, M and S. What we want at the output is equal
 *L, M and S
 * for the reference color (making it a neutral illuminant), or sL ref_L = sM
 *ref_M = sS ref_S.
 * This removes two degrees of freedom for our system, and we only need to find
 *fL.
 *
 * A reasonable last constraint would be to preserve Y, approximately the
 *brightness,
 * for the reference color. Since L'=M'=S' and the Y row of the LMS-to-XYZ
 *matrix
 * sums to unity, we know that Y'=L', and it's easy to find the fL that sets
 *Y'=Y.
 */
