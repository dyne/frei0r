 ...
 *
 * This effect simulates «natural vignetting» whose light falloff can be
 * described
 * with a cos⁴ curve. Additionally the x:y aspect ratio of the vignette can be
 * changed (note that normal cameras with a round aperture always have an aspect
 * ratio
...
private:
  ...
  void updateVignette()
  {
//        std::cout << "New settings: aspect = " << m_aspect << ", clear center
// = " << m_cc << ", soft = " << m_soft << std::endl;
    ...
    // Map scale to [0,5] in a way that values near 0 can be adjusted more
    // precisely
    scale = 1 + 4 * std::pow(scale, 3);
    // Scale either x or y, depending on the aspect value being above or below
    // 0.5
    if (m_aspect > 0.5)
    {
      scaleX = scale;
...
//        std::cout << "Used values: soft=" << soft << ", x=" << scaleX << ",
// y=" << scaleY << std::endl;
