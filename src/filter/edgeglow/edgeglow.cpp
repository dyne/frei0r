                      ...
                      const uint32_t *in)
  {
    ...
    for (unsigned int y = 1; y < height - 1; ++y)
    {
      for (unsigned int x = 1; x < width - 1; ++x)
      {
        ...
        if (l > (lt = lthresh * 255.))
        {
          // if lightness > threshold, we add it to the lightness of the
          // original
        ...
        if (lredscale > 0. || l > lt)
        {
          ...
          else
          {
            ...
            else
            {
              if (max == G)
              {
                h = (unsigned int)(60. * ((float)B - (float)R) / ((float)max - (float)min) + 120.);
              }
              else
              {
                h = (unsigned int)(60. * (float)(R - G) / float(max - min) + 240.);
              }
            }

            if (l <= 0.5)
            {
              s = ((float)max - (float)min) / ((float)max + (float)min);
            }
            else
            {
              s = ((float)max - (float)min) / (2. - ((float)max + (float)min));
            }
          }
          ...
          if (tr < 1. / 6.)
          {
            g[0] = CLAMP0255(p + ((q - p) * 6. * tr));
          }
          else if (tr < 0.5)
          {
            g[0] = CLAMP0255((int32_t)q);
          }
          else if (tr < 2. / 3.)
          {
            g[0] = CLAMP0255(p + ((q - p) * 6. * (2. / 3. - tr)));
          }
          else
          {
            g[0] = CLAMP0255((int32_t)p);
          }

          if (tg < 1. / 6.)
          {
            g[1] = CLAMP0255(p + ((q - p) * 6. * tg));
          }
          else if (tg < 0.5)
          {
            g[1] = CLAMP0255((int32_t)q);
          }
          else if (tg < 2. / 3.)
          {
            g[1] = CLAMP0255(p + ((q - p) * 6. * (2. / 3. - tg)));
          }
          else
          {
            g[1] = CLAMP0255((int32_t)p);
          }

          if (tb < 1. / 6.)
          {
            g[2] = CLAMP0255(p + ((q - p) * 6. * tb));
          }
          else if (tb < 0.5)
          {
            g[2] = CLAMP0255((int32_t)q);
          }
          else if (tb < 2. / 3.)
          {
            g[2] = CLAMP0255(p + ((q - p) * 6. * (2. / 3. - tb)));
          }
          else
          {
            g[2] = CLAMP0255((int32_t)p);
          }
        }
        else
        {
